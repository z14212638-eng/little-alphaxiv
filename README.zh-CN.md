...
| `ANYSEARCH_API_KEY` | *(未设置)* | `web_search` 工具的 API key——通过 HTTP 调用 [anysearch](https://anysearch.com) MCP 服务器，让助手能找到 arXiv/OpenAlex/Semantic Scholar 漏掉的论文（IEEE/ACM/Springer、付费墙、只有 DOI 的）并回答非学术问题。未设置 → `web_search` 返回「未配置」，模型自动退回到学术搜索工具（不会崩溃）。与你的 Claude Code MCP 配置里的同名环境变量通用。Docker 下在 `deploy/.env` 中设置。 |
